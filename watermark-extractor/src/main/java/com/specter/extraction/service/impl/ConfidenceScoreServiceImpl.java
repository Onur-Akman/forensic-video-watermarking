...
public class ConfidenceScoreServiceImpl implements ConfidenceScoreService {
    ...
    public double calculate(WatermarkPayload payload) {
        ...
        int cw = 84;
        if (softBits.length != 252) {
            log.warn("Expected 252 soft bits, got {}", softBits.length);
            return 0.0;
        }
        ...
        for (int i = 0; i < cw; i++) {
            ...
            //   1) sum frame votes within each repetition slot to get one value per slot (r0, r1, r2)
            //   2) confidence = | r0 + r1 + r2 | / ( |r0| + |r1| + |r2| )
            // Frame averaging-then-absolute (instead of per-frame absolute) is the matched-filter form:
            // noise cancels across frames before magnitude is taken, matching the M3 acceptance thresholds.
        ...
        log.info("Confidence calculation (Contract §5.2 Matched-Filter) — final: {}", String.format("%.4f", confidence));
